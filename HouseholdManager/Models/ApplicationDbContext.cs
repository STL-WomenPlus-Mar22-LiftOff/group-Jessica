...
namespace HouseholdManager.Models
{
    public class ApplicationDbContext : IdentityDbContext<AppUser>
    {
        ...
        {
            Member member1 = new Member() { MemberId = 1, MemberType = "Admin", HouseholdId = 1, UserName = "defaultAdmin@yahoo.com", };
            builder.Entity<Member>().HasData(member1);
            Member member2 = new Member() { MemberId = 2, MemberType = "Member", HouseholdId = 1, UserName = "defaultUser@yahoo.com", };
            builder.Entity<Member>().HasData(member2);
        }

        private static void SeedRoom(ModelBuilder builder)
        {
            Room room1 = new Room() { RoomId = 1, Name = "Kitchen", };
            builder.Entity<Room>().HasData(room1);
            Room room2 = new Room() { RoomId = 2, Name = "Bathroom", };
            builder.Entity<Room>().HasData(room2);
            Room room3 = new Room() { RoomId = 3, Name = "Master Bedroom", };
            builder.Entity<Room>().HasData(room3);
            Room room4 = new Room() { RoomId = 4, Name = "Living Room", };
            builder.Entity<Room>().HasData(room4);
            Room room5 = new Room() { RoomId = 5, Name = "Bedroom", };
            builder.Entity<Room>().HasData(room5);
            Room room6 = new Room() { RoomId = 6, Name = "Guest Bedroom", };
            builder.Entity<Room>().HasData(room6);
            Room room7 = new Room() { RoomId = 7, Name = "Master Bathroom", };
            builder.Entity<Room>().HasData(room7);
            Room room8 = new Room() { RoomId = 8, Name = "Dining Room", };
            builder.Entity<Room>().HasData(room8);
            Room room9 = new Room() { RoomId = 9, Name = "Yard", };
